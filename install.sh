...
verify_installation() {
    ...
    # 记录已安装版本号和二进制 SHA256（用于后续完整性验证与版本固定）
    local binary_path
    binary_path=$(command -v hysteria 2>/dev/null || echo "")
    if [[ -n "$binary_path" && -f "$binary_path" && -n "$version" ]]; then
        local sha256
        sha256=$(sha256sum "$binary_path" 2>/dev/null | awk '{print $1}')
        local version_file="/etc/hysteria/.installed_version"
        local sha256_file="/etc/hysteria/.installed_sha256"
        echo "$version" > "$version_file"
        chmod 600 "$version_file"
        if [[ -n "$sha256" ]]; then
            echo "$sha256" > "$sha256_file"
            chmod 600 "$sha256_file"
            log_info "已记录安装版本 $version SHA256: ${sha256:0:16}..."
        fi
    fi
