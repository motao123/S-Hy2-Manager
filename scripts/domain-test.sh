...
test_all_domains_concurrent_silent() {
    ...
    local timeout_val
    timeout_val=$(get_adaptive_timeout "$network_quality")
...
show_test_results() {
    ...
    local results
    results=$(test_all_domains_concurrent)
...
get_best_domain() {
    local best_result
    best_result=$(test_all_domains_concurrent_silent | head -n 1)
...
test_domain_stability() {
    ...
    if [[ ${#results[@]} -gt 0 ]]; then
        ...
        local success_rate
        success_rate=$(( (${#results[@]} * 100) / test_count ))
        ...
        # 稳定性评级
        local stability_variance
        stability_variance=$((max_latency - min_latency))
...
interactive_domain_selection() {
    ...
    local results
    results=$(test_all_domains_concurrent_silent | head -n 15)
    ...
    for domain in "${custom_domains[@]}"; do
        ...
        (
            if result=$(test_domain_latency "$domain" "$timeout_val" 1); then
                ...
                latency=$(echo "$result" | awk '{print $1}')
                local dns_status
                dns_status=$(echo "$result" | awk '{print $3}')
                local dns_display
                if [[ "$dns_status" == "dns_ok" ]]; then
                    dns_display="${GREEN}正常${NC}"
                else
...
manage_cache() {
    ...
    case $choice in
        1)
            ...
            ;;
        2)
            find "$CACHE_DIR" -mindepth 1 -maxdepth 1 -type f -delete 2>/dev/null
